...
for i in range(len(df)):
    try:
        ...
        if club_from == "" or club_to == "" or value == "" or value == "?" or value == "draft" or \
                value == "Available for free transfer" or value == "-" or value == "Free transfer":
            continue
        else:
            ...
            #league1 = League(id=df["from"][i]["leagueId"], href=df["from"][i]["leagueHref"], name=df["from"][i]["league"], country=df["from"][i]["country"])
            league2 = League(id=df["to"][i]["leagueId"], href=df["to"][i]["leagueHref"], name=df["to"][i]["league"], country=df["from"][i]["country"])
